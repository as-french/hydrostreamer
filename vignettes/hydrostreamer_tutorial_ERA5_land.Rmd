---
title: "hydrostreamer workflow using hourly gridded runoff and non-dentritic river networks"
...
"multi-thread") digital river networks. Specifically, the presented workflow uses
...
downscaling and routing methods (constant, instantaneous and Muskingum-Cunge)
and data temporal resolution (daily vs. hourly).

The ECMWF ERA5-Land data included in this package pertain to the extent of the
river Miera catchment in Spain as below (and the Shannon catchment in Ireland
for another example if desired) and were published by Copernicus under
...
contained in the package covering the extent of the Shannon system. GRUN data
were published under the CC By 4.0 license and are provided at a monthly
time-step.

The data sources included in the package can be substituted with modelled runoff
, e.g., using [`GR4J/GR4H`](https://hydrogr.github.io/airGR/) or similar to
translate gridded ECMWF ERA5 reanalysis (or SEAS5 seasonal forecast, or CMIP
projections) precipitation and evapotranspiration (or potential
evapotranspiration derived from surface temperature using the Hargreaves-Samani
method) into gridded runoff. Note however that this would require estimation of
subcatchment specific GR model parameters; hence, the application of
hydrostreamer to off-the-shelf climate data products. See data-raw folder of
this package for climate data pre-processing steps (e.g., decumulation of runoff
data).
...
classified as a "simple" dendritic acyclic graph. A larger example dataset is
provided for the river Shannon in Ireland, which contains several
anabranching/multi-thread channels and two outlets. These river networks, among
other compatible river systems, can be found at
https://doi.org/10.5281/zenodo.14366637 (French and McGinnity, 2026).
...
directed acyclic graphs). Each routing method works by dividing the total
...
and 9km grid resolution. Alternative sources of modelled runoff data include:
...
This tutorial demonstrates the first two of *hydrostreamer*'s workflow for
...
provides three river routing algorithms for this purpose:
**instantaneous** routing, useful for e.g. estimating runoff in the entire
upstream catchment of each river segment, **constant** flow velocity routing
(demonstrated here), and **Muskingum-Cunge**, which is currently not implemented to
work with multi-thread networks).
...
timestep. In this Miera catchment demonstration it is nearly all reaches that
require splitting.
...
`hydrostreamer::miera_rivers_split`. See also data-raw folder for qgisprocess
method.
...
and PREVIOUS list columns will need to be populated.

##### Regenerate attributes for subcatchments associated with split reaches

For regenerating subcatchments, we can use `hydrostreamer::river_voronoi`. And
elevation data can extracted from the packaged DEM and used for dasymetric
mapping.
...
The split river network requires new NEXT and PREVIOUS columns.
...
Two implementations of constant velocity routing are illustrated below. The
first one is vastly more computationally efficient but requires a two-step
process. The two step process is as follows: (i) we use
`hydrostreamer::split_network_at_bifurcations` to spit the network into
topologically ordered processing chunks; (ii) we use
`hydrostreamer::accumulate_runoff_constant_complex()` to route runoff in
topological sequence, i.e., upper catchment groups are routed first, and the
accumulated discharge time series calculated at each of these subgroup outlets
is then substituted into one or more of the upper reaches immediately downstream
of the first subgroup topologically, and so on.

The inefficient method does not split the network into topological groups.
Instead, a call to `hydrostreamer::accumulate_runoff()` will route runoff from
each reach through all possible downstream paths to outlets before averaging
discharge estimates for each reach. As a result of the averaging, there are
small differences in the results of this method vs. the computationally
inefficient one, but either method might be considered appropriate for a small
catchment that contains few bifurcations; hence our example for the Miera
catchment, which contains only one bifurcation.

Note, it is advisable to always use the efficient method and split the catchment
into appropriate topological chunks. This is for two main reasons. First, the
processing time will be excessive, and second, the method will fail due to
insufficient RAM. We provide example data for the Shannon catchment to
demonstrate that even the efficient method will take ~50 minutes to run after
having split the catchment into ~40 topologically ordered groups and a total of
~25 000 reaches to facilitate constant velocity routing. It is not advisable to
use instantaneous routing for hourly time steps, but may be appropriate for
monthly, or daily in some instances.

```{r plot_constant_velocity_routing_split_reaches_hourly}

# higher computational efficiency method splitting into distinct topologically
# ordered groups

RB_join_updated <-  hydrostreamer::split_network_at_bifurcations(RB_join,
    riverID = "riverID",
    verbose = TRUE)

RB_join_updated %>%
    ggplot()+
    labs(title = "Catchment split into topologically ordered groups",
        subtitle = "Constant velocity routing is applied sequentially to group 1, 2 and so on...") +
    geom_sf(aes(geometry = geom, col = factor(river_group_new)))+
    theme_bw()

# instantaneous
system.time({
river_discharge_instant_2 <- hydrostreamer::accumulate_runoff_instant_complex(
    sf_river_network = RB_join_updated,
    riverID = "riverID", 
...
# constant
system.time({
river_discharge_constant <- hydrostreamer::accumulate_runoff_constant_complex(
    sf_river_network = RB_join_updated,
    riverID = "riverID", 
    runoff_var_name = "ERA5_land",
...
# 41 seconds for 1 year of hourly time-step runoff for 832 river segments in
# Miera on standard laptop (2.10 GHz processor)

# ---------------------------------------------------------------------------- #

river_discharge_constant$mean_discharge <- 
    sapply(river_discharge_constant$discharge_ts, function(x)
    mean(x$ERA5_land))
...
river_discharge_constant %>%
    ...
    scale_color_gradient(name = "Discharge (m3/s)",
                         low = "#bdd7e7",
                         high = "#08519c") +
    labs(title = "Mean annual discharge",
         subtitle = "Routing method: Constant velocity") +
    theme_bw()

# ---------------------------------------------------------------------------- #
# NOTE low computational efficiency (and potentially incorrect) method, but will
# run for complex rivers
# system.time({
# river_discharge_constant_slow <-
...
We also present the Muskingum-Cunge method, which is now also applicable to
hourly runoff data and anabranching river networks.

```{r plot_muskingum_cunge_routing_split_reaches_hourly}

# higher computational efficiency method splitting into distinct topologically
# ordered groups

shallowest_grad = min(hydrostreamer::miera_rivers$grad[hydrostreamer::miera_rivers$grad > 0])

RB_join_updated <-  hydrostreamer::split_network_at_bifurcations(RB_join,
    riverID = "riverID",
    verbose = TRUE) %>%
    dplyr::left_join(.,hydrostreamer::miera_rivers %>%
                         sf::st_drop_geometry() %>%
                         dplyr::select("riverID", "grad") %>%
                         dplyr::rename("old_riverID" = "riverID"),
                     by = "old_riverID") %>%
    dplyr::mutate("grad" = dplyr::case_when(
        .data$grad <=0 ~ shallowest_grad,
        .default = .data$grad
    ))
...
system.time({
river_discharge_muskingum_cunge <- hydrostreamer::accumulate_runoff_muskingum_cunge_complex(
    sf_river_network = RB_join_updated,
    riverID = "riverID", 
    runoff_var_name = "ERA5_land",
    slope_var_name = "grad",
...
# 66 seconds for 1 year of hourly time-step runoff for 832 river segments in
# Miera on standard laptop (2.10 GHz processor)

# ---------------------------------------------------------------------------- #

river_discharge_muskingum_cunge$mean_discharge <- 
    sapply(river_discharge_muskingum_cunge$discharge_ts, function(x)
    mean(x$ERA5_land))

# https://colorbrewer2.org/#type=sequential&scheme=Blues&n=3
river_discharge_muskingum_cunge %>%
    ggplot() +
    geom_sf(aes(geometry = geom, col = mean_discharge)) +
    scale_color_gradient(name = "Discharge (m3/s)",
                         low = "#bdd7e7",
...
```{r plot_all_routing_time_series_hourly}
outlet_riverID <- river_discharge_constant$riverID[which((sapply(river_discharge_constant$NEXT, function(x){
    x[[1]] == -9999
}) %>%
    unlist()) == TRUE)]
...
river_discharge_con = river_discharge_constant %>%
 ...
 river_discharge_ins <- river_discharge_instant %>%
  ...
  river_discharge_ins_2 <- river_discharge_instant_2 %>%
    dplyr::filter(.data$old_riverID %in% outlet_old_riverID) %>%
    dplyr::slice_max(.data$UP_SEGMENTS) %>%
      tidyr::unnest("discharge_ts") %>%
    dplyr::mutate("routing_method" = "instant2")
 
  river_discharge_musk_c <- river_discharge_muskingum_cunge %>%
    dplyr::filter(.data$old_riverID %in% outlet_old_riverID) %>%
    dplyr::slice_max(.data$UP_SEGMENTS) %>%
    tidyr::unnest("discharge_ts") %>%
    dplyr::mutate("routing_method" = "muskingum_cunge")
    
river_discharge_methods <- river_discharge_con %>%
    dplyr::bind_rows(river_discharge_ins) %>%
    dplyr::bind_rows(river_discharge_musk_c) %>%
    dplyr::bind_rows(river_discharge_ins_2) %>%
...
# RANDOM reach
set.seed(1980)
random_riverID <- sample(river_discharge_constant$riverID,size = 1)

random_riverID_old_riverID <- river_discharge_constant %>%
    dplyr::filter(.data$riverID == random_riverID) %>%
    dplyr::pull("old_riverID")

# constant routing
river_discharge_con = river_discharge_constant %>%
    dplyr::filter(.data$old_riverID %in% random_riverID_old_riverID) %>%
    dplyr::slice_max(.data$UP_SEGMENTS) %>%
    tidyr::unnest("discharge_ts") %>%
    dplyr::mutate("routing_method" = "constant")

 river_discharge_ins <- river_discharge_instant %>%
    dplyr::filter(.data$riverID %in% random_riverID_old_riverID) %>%
    tidyr::unnest("discharge_ts") %>%
    dplyr::mutate("routing_method" = "instant")
...
data are aggregated to daily time-step for comparison. See data-raw for
aggregation method using `terra::tapp`.
...
# 12 seconds for 1 year of daily time-step runoff for 832 river segments in
# Miera on standard laptop (2.10 GHz processor)
...
```{r plot_muskingum_cunge_routing_split_reaches_daily}
...
# 10 seconds for 1 year of daily time-step runoff for 832 river segments in
# Miera on standard laptop (2.10 GHz processor)
...
widths and depths) from approximate relationships (e.g., Pistocchi and
Pennington (2006); Morel et al., (2020)), which could be useful for estimating
habitat availability (e.g., for salmonids, see French and McGinnity (2026)).

Potential future work might include:

(i) Development of an automated method for splitting reaches to facilitate
constant velocity runoff routing at hourly resolution (i.e., less reliance on
qgisprocess or manual editing);

(ii) Comparison and statistical bias adjustment of reach scale estimates against
gauge data (e.g., Empirical Quantile Mapping);

(iii) Routing discharge at alternative temporal resolutions, such as the 3-6
...
French, A.S., McGinnity P. (2026) AMBER Salmon: A digital river network
containing reach-specific habitat variables related to the North-East Atlantic
distribution of Atlantic salmon (Salmo salar). [Dataset]
https://doi.org/10.5281/zenodo.14366637
