...
required <- c("exactextractr","terra","ggplot2","classInt")
...
These data could be substituted with modelled runoff using GR4J or similar to
translate gridded ECMWF ERA5 reanlaysis (or SEAS5 seasonal forecast, or CMIP
projections) precipitation (potential evapotranspiration dervied from surface
temperature using the Hargreave's Samani method).
...
* ~45m resolution (~2.3arc second DEM) located in the west of Ireland. The DEM is originally Mapzen Terrain Tiles, accessed using *elevatr*.
...
# in degrees
#shannon_DEM_deg = shannon_DEM %>%
#    terra::rast() %>%
#    terra::project("EPSG:4326")
#    res(shannon_DEM_deg) # 0.0006399219

# celestial::deg2dms(deg = res(shannon_DEM_deg))
# DEG   MIN  SEC    
# "+00" "00" "02.30"
#NOTE used shannon_DEM = shannon_DEM * 1 then usethis::use_data to commit raster
#to memory before exporting into R package. This way 8mb raster does not need to
#be stored in extdata, just smaller rda file in /data/.
plot(shannon_DEM)
plot((sf::st_union(hydrostreamer::shannon_subcatchments)), add = TRUE)
```

* ~40km resolution (~0.6 degree) runoff timeseries in the same area as the DEM. Runoff is sourced from GRUN. The unit of runoff is mm/day, and are provided with a monthly timestep.
...
* A river network derived from the provided DEM with 729 river segments.
...
* River segment specific catchment areas, delineated from the provided DEM. Catchments are provided for all (n = 729) of the river segments.
...
```{r look_at_data, message=FALSE, warning=FALSE}
...
plot((shannon_rivers), add=TRUE, lwd = .5)
```
...
```{r raster_to_HS, message=FALSE, warning=FALSE}
library(dplyr)
library(terra)
library(raster)
...
#set nas to zero
shannon_GRUN.rast.rcls <- raster::reclassify(hydrostreamer::shannon_GRUN, cbind(NA, 0))
...
source_runoff <- hydrostreamer::raster_to_HS(rasters = shannon_GRUN.rast.rcls, 
                       unit = "mm/day",
...
[create_HS](https://mkkallio.github.io/hydrostreamer/reference/create_HS.html).
...
#AWI$mean_runoff <- sapply(AWI$runoff_ts, function(x) mean(x$GRUN))
...
elevation_values <- exactextractr::exact_extract(x = shannon_DEM,
                                   y = basins,
                                   fun = "mean",
                                   append_cols = c("riverID"), progress = FALSE) %>%
...
PPDM <- interpolate_runoff(HS = HS,
                         ...
                         riverID = "riverID",
                         verbose = FALSE)
PPDM$mean_runoff <- sapply(PPDM$runoff_ts, function(x) mean(x$GRUN))
...
A2L <- interpolate_runoff(HS = HS,
                          river = rivers,
                          riverID = "riverID",
                          verbose = FALSE)

A2L$mean_runoff <- sapply(A2L$runoff_ts, function(x) mean(x$GRUN))
plot(A2L[,"mean_runoff"])

# DASYMETRIC MAPPING WITH LINES

A2LDM <- interpolate_runoff(HS = HS,
                            river = rivers,
                            dasymetric = "gmean_m",
                          riverID = "riverID",
                          verbose = FALSE)

A2LDM$mean_runoff <- sapply(A2LDM$runoff_ts, function(x) mean(x$GRUN))
...
RB$mean_runoff <- sapply(RB$runoff_ts, function(x) mean(x$GRUN))
plot(RB[,"mean_runoff"])
```
...
system.time({
    ...
    river_network(hydrostreamer::shannon_rivers, riverID = "riverID", verbose = FALSE)
})
...
PPDM.join = dplyr::left_join(PPDM,routed_river_hydrostreamer_NPID, by = "riverID")

RB.join = dplyr::left_join(RB,routed_river_hydrostreamer_NPID, by = "riverID")
```


```{r plot_routing1, message=FALSE, warning=FALSE}
catchment_discharge <-
    hydrostreamer::accumulate_runoff(PPDM.join, routing_method = "instant", verbose = FALSE)
river_discharge <- hydrostreamer::accumulate_runoff(RB.join, routing_method = "instant", verbose = FALSE)

#catchment_discharge$mean_discharge <- sapply(catchment_discharge$discharge_ts, function(x) mean(x$GRUN))
catchment_discharge$mean_discharge <- sapply(catchment_discharge$discharge_ts, function(x){
  # filter may june
  x$month = lubridate::month(x$Date)# %>%
...
river_discharge$mean_discharge <- sapply(river_discharge$discharge_ts, function(x){
  # filter may june
  x$month = lubridate::month(x$Date)# %>%
  mean_discharge = mean(x$GRUN[(x$month %in% c(5,6))])
  return(mean_discharge)
...
library(ggplot2)
my_breaks = quantile(river_discharge$mean_discharge, c(0,0.1,0.2,0.4,0.8,1))

my_breaks <-
    classInt::classIntervals(river_discharge$mean_discharge,
                             n = 5,
                             style = "fisher")$brks
...
#    scale_colour_viridis_c(name = "Discharge",guide = "legend",
#                           breaks = my_breaks, labels = my_breaks) +
    scale_color_gradient(name = "Discharge (m3/s)",
                         low = "#bdd7e7",high = "#08519c")+
    theme_bw()
