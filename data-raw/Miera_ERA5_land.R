## code to prepare `miera_ERA5_land` dataset
...
# ---------------------------------------------------------------------------- #
# DOWNLOAD ERA5-LAND data for the MIERA CATCHMENT, IRELAND
dirName <- paste0(system.file("extdata", package = "hydrostreamer"),
                  "/ERA5/")
dir.create(dirName, showWarnings = TRUE, mode = "0777")

river_net_catchments <- hydrostreamer::miera_rivers

subcatchments <- hydrostreamer::miera_subcatchments
...
system.time({
    processed_rasters <- pbapply::pblapply(1:length(runoff_subset_list_yrs), function(k){
        ...
        # 2 minutes per year of data if Shannon sized catchment
        # 20 seconds for Miera sized catchments
...
miera_ERA5 <- terra::crop(x = runoff,y = e, snap = "out")
...
# export to file
terra::writeRaster(miera_ERA5, paste0(dirName, "miera_ERA5.tif"),
                   overwrite = TRUE)

miera_ERA5 = raster::brick(paste0(dirName, "miera_ERA5.tif"))

miera_ERA5 = miera_ERA5 * 1

miera_ERA5_land <- raster::setZ(miera_ERA5, times[,1], "Date")
names(miera_ERA5_land) <- times[,1]

usethis::use_data(miera_ERA5_land, overwrite = TRUE)

# --------------------------------------------------------------------------- #
# DAILY
# --------------------------------------------------------------------------- #

year_range = "2023_2023"

# read as raster
ERA5_Land_NC_runoff13 <- list(c(paste0(dirName,"surface_runoff_daily_8dp_",
                                       catchment_name, "_", year_range, ".nc")))
...
# export to file
terra::writeRaster(miera_ERA5, paste0(dirName, "miera_daily_ERA5.tif"),
                   overwrite = TRUE)

miera_ERA5 = raster::brick(paste0(dirName, "miera_daily_ERA5.tif"))

miera_ERA5_land_daily = miera_ERA5 * 1

miera_ERA5_land_daily <- raster::setZ(miera_ERA5_land_daily, times[,1], "Date")
names(miera_ERA5_land_daily) <- times[,1]

usethis::use_data(miera_ERA5_land_daily, overwrite = TRUE)
