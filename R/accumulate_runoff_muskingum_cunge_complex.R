#' Apply iterative Muskingum-Cunge routing to a pre-processed complex river
#' network
#' 
#' A method for Muskingum-Cunge velocity routing that takes a pre-processed HS
#' river network that contains reach specific interpolated rainfall runoff and
#' routes runoff though the network according to topologically ordered grouping.
#'
...
#' @param slope_var_name Character string of column name for reach slope.
#' @param manning_var_name Character string of column name for reach Manning
#'   value.
...
#'   hydrostreamer::accumulate_runoff_muskingum_cunge
#'
...
#' 
#' Note also that the default arguments for the
#' accumulate_runoff_muskingum_cunge are currently used for this method, so only
#' reach slope (and manning values if desired) need to be supplied. Individual
#' reach widths is not implemented for this function, so the "Moody" default is
#' used.
#'   
#' @return Returns a HS river network with discharge_ts list column
#' @export
accumulate_runoff_muskingum_cunge_complex <- function(
        sf_river_network,
        riverID = "riverID",
        runoff_var_name = "ERA5_land",
        slope_var_name = "grad",
        manning_var_name = NULL,
    ...
    for(i in (1:length(bifurcation_sub_groups.processing.list))){
        ...
        # HS = bifurcation_sub_groups.processing.list[[i]]
        # manning = bifurcation_sub_groups.processing.list[[i]][[manning_var_name]]
        # slope = bifurcation_sub_groups.processing.list[[i]][[slope_var_name]]
        # verbose = TRUE
        
        # run cv routing
        bfur_subcat_group_i <- hydrostreamer::accumulate_runoff_muskingum_cunge(
            HS = bifurcation_sub_groups.processing.list[[i]],
            manning = bifurcation_sub_groups.processing.list[[i]][[manning_var_name]],
            slope = bifurcation_sub_groups.processing.list[[i]][[slope_var_name]],
            ...) %>% 
